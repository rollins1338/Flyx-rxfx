// Final attempt to crack the encoding with full data
...
console.log('🔓 FINAL ENCODING CRACK ATTEMPT');
console.log('='.repeat(70));
...
// This uses URL-safe base64 characters (_ and -)
console.log(`\nDetected: URL-safe base64 (uses _ and -)`);

// Method 1: Convert URL-safe to standard base64, reverse, then decode
console.log(`\n[1] URL-safe Base64 → Standard → Reverse → Decode:`);
try {
  ...
  if (decoded.includes('http')) {
    ...
    console.log(`\n   FULL URL: ${decoded}`);
    console.log(`\n${'='.repeat(70)}`);
    console.log('🎉 SUCCESS! ENCODING CRACKED!');
    console.log('='.repeat(70));
    console.log(`\nMethod: URL-safe Base64 → Reverse → Decode`);
    console.log(`\nImplementation:`);
...
// Method 2: Reverse first, then convert and decode
console.log(`\n[2] Reverse → URL-safe to Standard → Decode:`);
try {
  // Reverse first
  const reversed = encoded.split('').reverse().join('');
  
  // Convert URL-safe to standard
  let standard = reversed.replace(/_/g, '/').replace(/-/g, '+');
...
// Method 3: Just decode URL-safe base64 directly
console.log(`\n[3] Direct URL-safe Base64 Decode:`);
try {
  let standard = encoded.replace(/_/g, '/').replace(/-/g, '+');
  const decoded = Buffer.from(standard, 'base64').toString('utf8');
