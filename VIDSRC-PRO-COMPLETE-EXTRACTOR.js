/**
 * VIDSRC PRO.RCP COMPLETE AUTOMATED EXTRACTOR
 * Extracts M3U8 URLs from vidsrc-embed.ru using the div ID variable method
 */
...
class VidsrcProExtractor {
  ...
  async extractFromEmbed(embedUrl) {
    ...
    try {
      ...
      if (!divInfo) {
        throw new Error('Hidden div not found on page');
      }
      ...
      const m3u8Url = await page.evaluate((divId) => {
   ...
   */
  async extractWithRetry(type, tmdbId, season = null, episode = null, maxRetries = 3) {
    let lastError;
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    console.log('Usage:');
    console.log('  Movie:    node VIDSRC-PRO-COMPLETE-EXTRACTOR.js movie <tmdbId>');
    console.log('  TV Show:  node VIDSRC-PRO-COMPLETE-EXTRACTOR.js tv <tmdbId> <season> <episode>');
    console.log('');
    console.log('Examples:');
    console.log('  node VIDSRC-PRO-COMPLETE-EXTRACTOR.js movie 550');
    console.log('  node VIDSRC-PRO-COMPLETE-EXTRACTOR.js tv 1396 1 1');
    process.exit(1);
  }
