// Final working decoder based on analysis
...
console.log('🔓 FINAL WORKING DECODER');
console.log('='.repeat(70));

console.log(`\nDiv ID: ${divId}`);
console.log(`Encoded length: ${encoded.length}`);

// Step 1: URL-safe base64 to standard
...
// Now try different XOR keys
const keys = [
  { name: 'divId', value: divId },
  { name: 'divId MD5', value: crypto.createHash('md5').update(divId).digest() },
  { name: 'divId SHA256', value: crypto.createHash('sha256').update(divId).digest() },
  { name: 'divId lowercase', value: divId.toLowerCase() },
  { name: 'divId uppercase', value: divId.toUpperCase() },
  { name: 'divId reversed', value: divId.split('').reverse().join('') },
  { name: 'Static "playerjs"', value: 'playerjs' },
  { name: 'Static "pjsdiv"', value: 'pjsdiv' },
  { name: 'Static "file"', value: 'file' },
];

console.log(`\nTrying ${keys.length} different XOR keys...\n`);

for (const keyInfo of keys) {
  const key = typeof keyInfo.value === 'string' ? Buffer.from(keyInfo.value) : keyInfo.value;
  
  const xored = Buffer.alloc(base64Decoded.length);
  for (let i = 0; i < base64Decoded.length; i++) {
    xored[i] = base64Decoded[i] ^ key[i % key.length];
  }
  
  const result = xored.toString('utf8');
  
  if (result.includes('http')) {
    console.log(`✅✅✅ SUCCESS with key: ${keyInfo.name} ✅✅✅`);
    console.log(`\nFULL URL: ${result}\n`);
    console.log('='.repeat(70));
    console.log('🎉 ENCODING CRACKED!');
    console.log('='.repeat(70));
    console.log(`\nMethod:`);
    ...
    console.log(`5. XOR with key: ${keyInfo.name}`);
    process.exit(0);
  }
}

console.log('❌ None of the keys worked');
console.log('\nThe XOR key must be something else or there\'s an additional step.');
console.log('Recommendation: Use Puppeteer to intercept the decoded URL from the browser.');
